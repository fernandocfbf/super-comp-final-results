...
result compute_score_for_all(vector<string> a_all, vector<string> b_all){
    ...
    for (auto& sub_a: a_all){
        for (auto& sub_b: b_all){
            ...
            new_struct.s1 = sub_a; 
            new_struct.s2 = sub_b;
            scores_list.push_back(new_struct);
        }
    }

    #pragma omp parallel for private(s1_aligned, s2_aligned, score) num_threads(4)
    for (auto& par_sub: scores_list){
        string sub_a = par_sub.s1;
        string sub_b = par_sub.s2;
        run_p1(sub_a, sub_b, s1_aligned, s2_aligned, (int)sub_a.size(), (int)sub_b.size(), score);
        par_sub.score = score.value;
    }

    for (auto& obj: scores_list){

        if (obj.score > best_alignment.score){
            best_alignment.score = obj.score;
            best_alignment.s1 = obj.s1;
            best_alignment.s2 = obj.s2;
        }
    }
